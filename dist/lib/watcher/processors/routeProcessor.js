    ...
    try {
        ...
        ts.forEachChild(sourceFile, (node) => {
            ...
            for (const decl of node.declarationList.declarations) {
                ...
                if (!exprName.endsWith("wrapHandler"))
                    continue;
        ...
        for (const [routePath, routeGroup] of routesByPath) {
            ...
            const moduleRoutePath = path.join(WATCHER_CONFIG.MODULES_DIR, moduleName, `${moduleName}.route`);
            ...
            const exports = routeGroup
                .map((r) => `export { ${r.exportName} as ${r.method.toUpperCase()} } from "${safeImport}";`)
                .join("\n");
            fs.writeFileSync(apiFile, exports + "\n", "utf8");
            log.success(`✓ Created ${routePath}/route.ts with ${routeGroup.length} method(s)`);
        }
        log.success(`Processed ${routes.length} route(s) for module: ${moduleName}`);
    }
    catch (err) {
        log.error(`Failed to process ${filePath}: ${err instanceof Error ? err.message : String(err)}`);
    }
}
