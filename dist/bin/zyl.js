...
async function createServerStructure() {
    ...
    if (!fs.existsSync(baseCoreDir))
        generateCore(name, "boilerPlate");
    if (!fs.existsSync(baseModuleDir))
        generateModule(name, "boilerPlate");
...
if (cmd === "server" && subcmd === "init") {
    ...
    if (cmd === "g" && subcmd === "module")
        generateModule(name, "new");
    else if (cmd === "g" && subcmd === "core")
        generateCore(name, "new");
}
else if (cmd === "watch") {
    if (!isServerInitialized()) {
